...
@Slf4j
public class DemoDataListener implements ReadListener<Sheet1> {
     ...
     * 缓存的数据
     */
    private List<Sheet1> cachedDataList = ListUtils.newArrayListWithExpectedSize(BATCH_COUNT);

    /**
     * 假设这个是一个DAO，当然有业务逻辑这个也可以是一个service。当然如果不用存储这个对象没用。
     */

    public DemoDataListener(String target) {
    ...
    private void saveData() {
        for (Sheet1 sheet1 : cachedDataList) {
    ...
    private String getApplications(Sheet1 sheet1) {
